...
while [[ $userguess != $secretnumber ]] ;do # ask repeatedly until they get it right
  if [[ $userguess = '' ]]; then echo "Empty"; fi
  if [[ $userguess -lt 1 || $userguess -gt 10 ]]; then echo "Outside of range"; fi
  if [[ $userguess -lt $secretnumber ]]; then echo "Too Low"; fi
  if [[ $userguess -gt $secretnumber ]]; then echo "Too High"; fi
